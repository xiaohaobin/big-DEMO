...
function queryDatabase(p,callback){
	...
	if(p.keyword == ""){
		var _page_sql = page_sql + (pageNum * 10 - 10) + "," + pageSize;//无模糊检索
	}else{//模糊检索
		var _page_sql = ' SELECT * FROM diy_user WHERE CONCAT(name,email,address) LIKE "%'+ p.keyword +'%" LIMIT ' + (pageNum * 10 - 10) + "," + pageSize;
	}
